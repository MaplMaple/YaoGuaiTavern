...
public class BarOpening : MonoBehaviour
{
    ...
    private void PlayOpeningDialogue()
    {
        ...
        dialogTexts.Add(new DialogData(originalString: "几天前，我收到了我的朋友木樨发来的一封信件。", character: "主角"));
        dialogTexts.Add(new DialogData(originalString: "虽然没有寄出者的信息，但是我很确信这是她寄来的，因为随信而来的还有我在学生时代赠送给她的护身符。", character: "主角"));
        dialogTexts.Add(new DialogData(originalString: "信的内容几乎完全由我看不懂的文字进行书写，但落款处留下了一个地址，指向雨城的一家小酒馆。", character: "主角"));
        dialogTexts.Add(new DialogData(originalString: "为了调查这封信以及木樨发这封信给我的原因，我启程前往雨城。至于木樨那边到底发生了什么事，也许只有我到达酒馆之后才能知道了。", callback: () => { FadeInPlayer().Forget(); }, character: "主角"));
    ...
    private void PlayerMonologue()
    {
        ...
        dialogTexts.Add(new DialogData(originalString: "/emote:Shock/喂，你一个人去？现在都几点了，楼顶黑灯瞎火的。", character: "阿枫"));
        ...
        dialogTexts.Add(new DialogData(originalString: "放心吧，我只是上去看看而已。", character: "主角", callback: () => { LoadScene(); }));
        DialogManager.Show(dialogTexts);
    }
